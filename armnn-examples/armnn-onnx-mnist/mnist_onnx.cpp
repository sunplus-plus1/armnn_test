...
int main(int argc, char** argv)
{
    ...
    //armnn::INetworkPtr network = parser->CreateNetworkFromBinaryFile("model/model.onnx",
    //                                                               { }, // input taken from file if empty
    //                                                               { "prob" }); // output node
    armnn::INetworkPtr network = parser->CreateNetworkFromBinaryFile("model/model.onnx");

    // Find the binding points for the input and output nodes
    armnnOnnxParser::BindingPointInfo inputBindingInfo = parser->GetNetworkInputBindingInfo("Input3");
    armnnOnnxParser::BindingPointInfo outputBindingInfo = parser->GetNetworkOutputBindingInfo("Plus214_Output_0");
